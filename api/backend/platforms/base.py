...
def build_ydl_opts(overrides=None, platform=None, progress_hooks: Optional[List] = None, cachedir: Optional[bool] = None):
    ...
    if cookies_file and os.path.exists(cookies_file):
        # Validate Netscape format to avoid yt-dlp hard failures
        try:
            with open(cookies_file, 'r', encoding='utf-8', errors='ignore') as cf:
                head = cf.read(4096)
            looks_netscape = ('# Netscape HTTP Cookie File' in head) or ('\t' in head and not head.strip().startswith('{'))
            if looks_netscape:
                ...
                if _safe_bool(os.environ.get('STRICT_COOKIES'), False):
                    ...
                    logger.warning("Invalid cookies file format detected; skipping cookiefile. Set STRICT_COOKIES=1 to force usage.")
